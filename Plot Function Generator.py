...
NAMESPACES = {
    ...
    (-3,-5): ("ischool04_plot", 0, True, "minecraft:piglin_head", "The Bloody Valley", "Gather your courage to fight strong warriors in the Bloody Valley, in order to unlock the powerful overworld portal.\n\n§6ischool04 & Dominexis"),
    ...
    (-1,-4): ("kirnt", 0, False, "minecraft:oxidized_copper", "Stellar Adventure", "Leap through outer space from world to world!\n\n§6kirnt2027 & Minigame Makers"),
    ...
    (-1,-1): ("lory", 1, True, "minecraft:chiseled_sandstone", "Oasis", "Manage to parkour your way out of the oasis!\n\n§6_Lory44 & CosmicAxolotl"),
    ...
    (2,5): ("kello", 0, True, "minecraft:diamond_horse_armor", "Ancient Apparatus", "The strange and unfamiliar aura of the temple hypnotizes and taking all of your interest towards its majestic mechanism, with no malicious intent.\n\n§6KelloVerra", '{CustomModelData:351903}'),
    ...
    (4,-3): ("uk", 0, True, "minecraft:target", "Puzzle Escape", "Parkour and solve puzzles to escape!\n\n§6That_UK & Dominexis"),
    ...
    (4,1): ("piemel", 2, True, "minecraft:amethyst_shard", "Playground", "Is it a bird? Is it a plane? No! It's the remains of the past.\n\n§6PiemelPiraat & MaSp005 & Owca & _TheSwagUnicorn_ & Dominexis"),
    (4,2): ("hhyes", 0, False, "minecraft:slime_block", "Escape From Within", "Escape the reverse dropper!\n\n§6Hhyes & KaiWsMC & Dominexis"),
    (4,3): ("big_kitchen", 1, True, "minecraft:cooked_beef", "Kitchen Climb", "It's a big world out there!\n\n§6SnakeRattler & _TheSwagUnicorn_"),
...
for coordinate in coordinates:
    commands.append(
        f'execute if score @s pr.plot_x matches {coordinate[0]} if score @s pr.plot_z matches {coordinate[1]} if entity @s[tag=!pr.was_in_lobby] unless score @s spectate matches 1.. unless score @s lobby matches 1.. unless score @s pr.plot = #spawn_plot pr.value run advancement grant @s only pr:plot_{str(coordinate[0]).replace("-", "n")}_{str(coordinate[1]).replace("-", "n")}'
    )
...
for coordinate in coordinates:
    ...
    if len(plot) < 6:
        continue
    authors: list[str] = plot[5].split("§6")[-1].split(" & ")
    for author in authors:
        if author not in mapmakers:
            mapmakers.append(author)

mapmakers.append("CosmicCortexSP")

mapmakers.sort()

with (PROGRAM_PATH / "Mapmakers.txt").open("w", encoding="utf-8", newline="\n") as file:
    file.write("\n".join(mapmakers))

...
execute if score #boolean pr.value matches 0 run tellraw @s {"text":"You've been warped to the main build zone","color":"gold","type":"text"}
execute if score #boolean pr.value matches 1 run tellraw @s {"text":"You've been warped to the temporary build zone","color":"gold","type":"text"}

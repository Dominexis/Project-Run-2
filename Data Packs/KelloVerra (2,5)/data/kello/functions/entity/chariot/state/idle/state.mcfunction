...
execute if entity @e[type=marker,tag=pr.target,tag=!pr.ignore,tag=kello.entity.type.destination_marker,tag=kello.entity.target,distance=..1.5] run function kello:entity/chariot/state/lock/start
...
execute as @e[type=#kello:generic/system,distance=..64,tag=pr.target,tag=!pr.ignore,tag=kello.decal.controlled,tag=kello.decal.controller.parkour] if score @s kello.decal.id = #local kello.decal.id if score @s kello.entity.id = #local kello.entity.id at @s run function kello:entity/decal/parkour_controller/state/idle/children/attach_anim

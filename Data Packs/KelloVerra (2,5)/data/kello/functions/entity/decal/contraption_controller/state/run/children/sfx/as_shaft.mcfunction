...
execute as @e[type=item_display,tag=pr.target,tag=!pr.ignore,tag=!kello.exclude,tag=kello.entity.type.thrown_cogwheel,distance=..16,sort=nearest,limit=1] run scoreboard players operation #local kello.player.id = @s kello.player.id

...
execute if entity @s[tag=kello.entity.decal_controller_idle_init] as @e[type=#kello:generic/decal,distance=..64,tag=pr.target,tag=!pr.ignore,tag=kello.decal.controlled,tag=kello.decal.controller.contraption] if score @s kello.decal.id = #local kello.decal.id positioned as @s run function kello:entity/decal/contraption_controller/state/idle/children/init/main

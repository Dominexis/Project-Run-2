...
execute if entity @e[type=item_display,tag=pr.target,tag=!kello.ignore,tag=!kello.exclude,tag=kello.entity.type.thrown_cogwheel,distance=..1,sort=nearest,limit=1] run function kello:entity/decal/contraption_controller/state_optional/run/children/reset/decwheel_0

...
execute if score @s kello.entity.checkpoint.id = @s kello.entity.checkpoint.id unless entity @s[tag=kello.checkpoint.is_loaded] run function kello:entity/decal/optional_contraption_controller/state/out/start
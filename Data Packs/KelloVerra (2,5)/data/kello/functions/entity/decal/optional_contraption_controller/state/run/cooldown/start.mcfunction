...
execute as @e[type=marker,tag=pr.target,tag=!pr.ignore,tag=kello.entity.type.time_mark,tag=kello.entity.part.mark,distance=..256] if score @s kello.decal.id = #local kello.decal.id positioned as @s run function kello:entity/time_mark/spawn/main

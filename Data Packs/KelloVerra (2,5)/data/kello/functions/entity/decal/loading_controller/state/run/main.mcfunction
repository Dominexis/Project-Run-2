...
execute if score @s kello.entity.timer matches 1.. on passengers if entity @s[type=snowball,tag=kello.decal.pointer_origin] on origin if score @s kello.decal.id = #local kello.decal.id rotated as @s positioned as @s run function kello:entity/decal/loading_controller/state/run/children/main

...
execute as @e[type=item_display,tag=pr.target,tag=!pr.ignore,tag=aj.escapement.root,scores={kello.entity.quantity=1..},distance=..6,sort=nearest,limit=1] run function kello:entity/thrown/small_cog/state/puller/as_detach

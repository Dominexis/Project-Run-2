...
execute as @e[type=item_display,tag=pr.target,tag=!kello.ignore,tag=kello.entity.type.groundicator,tag=kello.entity.type.small,distance=...1,sort=nearest] run tag @s add kello.dead
...
execute as @e[type=item_display,tag=pr.target,tag=kello.entity.type.decal_controller] if score @s kello.decal.id = #local kello.decal.id run function kello:entity/thrown/small_cog/state/fixed/as_controller
...
execute as @e[type=minecraft:item_display,tag=pr.target,tag=kello.entity.type.decal_controller] if score @s kello.decal.id = #local kello.decal.id run function kello:entity/thrown/small_cog/state/fixed/as_controller_detach
return 1
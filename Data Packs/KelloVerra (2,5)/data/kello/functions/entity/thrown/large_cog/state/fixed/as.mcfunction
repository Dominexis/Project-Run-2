...
execute as @e[type=minecraft:item_display,distance=..1,tag=pr.target,tag=!pr.ignore,tag=kello.entity.type.groundicator,tag=kello.entity.type.large,sort=nearest] run tag @s add kello.dead
...
execute as @e[type=minecraft:item_display,tag=pr.target,tag=kello.entity.type.decal_controller] if score @s kello.decal.id = #local kello.decal.id run function kello:entity/thrown/large_cog/state/fixed/as_controller
return 1
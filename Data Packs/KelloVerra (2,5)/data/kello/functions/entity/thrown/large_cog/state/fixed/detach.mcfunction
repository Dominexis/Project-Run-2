...
execute as @e[type=item_display,tag=pr.target,tag=!pr.ignore,tag=kello.entity.type.cogwheel_shaft,tag=kello.entity.type.large,tag=kello.attach,distance=..24,sort=nearest,limit=1] at @s run function kello:entity/thrown/large_cog/state/fixed/as_detach

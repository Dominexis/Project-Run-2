execute as @e[type=marker,tag=pr.target,tag=!pr.ignore,tag=kello.entity.type.destination.marker,distance=..128] if score @s kello.player.id = #local kello.player.id run kill @s

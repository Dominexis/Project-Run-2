...
execute as @a[tag=pr.target] if entity @s[tag=dtplayers.riding_rocket] at @s as @e[type=minecraft:armor_stand,tag=dtplayers.rocket] if score @p[tag=pr.target] dtplayers.rocket.id = @s dtplayers.rocket.id run function dtplayers:rocket/rocket_tick

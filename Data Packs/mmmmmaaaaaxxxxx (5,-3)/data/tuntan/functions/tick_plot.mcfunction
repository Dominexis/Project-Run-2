...
execute as @a[tag=tuntan.enoughpoints,tag=pr.target,scores={tuntan_clicked=1..}] run function pr:player/finish
execute as @a[tag=tuntan.enoughpoints,tag=pr.target,scores={tuntan_clicked=1..}] run function tuntan:exit

execute as @a[tag=pr.target] run function tuntan:display_timer
effect clear @e[tag=pr.target] minecraft:levitation
execute positioned ~-19 ~121 ~-19 run effect give @e[dx=38,dy=17,dz=38,tag=pr.target] minecraft:levitation infinite 69 true
execute as @a[tag=pr.target,predicate=tuntan:onred] run function tuntan:tostart
item replace entity @a[tag=pr.target,tag=tuntan.started] weapon.offhand with minecraft:firework_rocket{Fireworks:{Flight:1b}}
...
execute at @e[tag=tuntan.finish,tag=pr.target,tag=!tuntan.teleport] run particle minecraft:block minecraft:gold_block ~ ~ ~ 0.3 0.3 0.3 0 10
...
item replace entity @e[tag=tuntan.enoughpoints,tag=pr.target] hotbar.4 with minecraft:carrot_on_a_stick{display:{Name:'{"text":"CLICK THIS TO FINISH MAP","italic":false,"type":"text"}',Lore:['{"text":"Feel free to play the rest of the tunnels.","color":"white","italic":false,"type":"text"}','{"text":"You can earn up to 50 points by completing all 10!","color":"white","italic":false,"type":"text"}']}}
return 1
...
kill @e[type=minecraft:horse,tag=pr.target,scores={Swag.HorsePing=0}]

#Anti-Horse Dismounting
execute as @a[tag=pr.target,scores={Swag.HorsePing=0}] if score @s Swag.TickDelay matches 0 at @s run kill @e[type=minecraft:horse,tag=pr.target,sort=nearest,limit=1]
...
execute as @a[tag=pr.target,scores={Swag.HorsePing=0}] if score @s Swag.TickDelay matches 2 at @s run summon minecraft:horse ~ ~ ~ {Silent:1b,Tame:1b,Variant:1029,Tags:["pr.target"],Attributes:[{Name:"minecraft:generic.max_health",Base:1.0d},{Name:"minecraft:generic.movement_speed",Base:0.5d},{Name:"minecraft:horse.jump_strength",Base:0.75d}],SaddleItem:{id:"minecraft:saddle",Count:1b}}
...
execute as @e[type=minecraft:horse,tag=pr.target] at @s if block ~ ~ ~ minecraft:water run kill @s

#Ending
execute as @a[tag=pr.target] at @s if block ~ ~ ~ minecraft:end_gateway on vehicle run kill @s

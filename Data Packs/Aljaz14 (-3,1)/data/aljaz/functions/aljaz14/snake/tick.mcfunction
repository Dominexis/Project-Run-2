...
execute if block ~-9 ~3 ~12 warped_pressure_plate[powered=true] run tag @e[type=armor_stand,tag=snake_3,tag=begin,tag=pr.target] add beginSnake
...
execute if block ~-9 ~3 ~12 warped_pressure_plate[powered=true] run tag @e[type=armor_stand,tag=snake_4,tag=begin,tag=pr.target] add beginSnake

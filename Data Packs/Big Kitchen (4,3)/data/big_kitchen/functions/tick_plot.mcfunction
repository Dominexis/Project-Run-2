...
execute as @a[tag=pr.target] at @s if block ~ ~ ~ minecraft:cave_air run kill @s

execute as @a[tag=pr.target] at @s if block ~ ~-1 ~ minecraft:coal_block run tag @s add big_kitchen.warp1
execute as @a[tag=pr.target,tag=big_kitchen.warp1] positioned ~37 ~17 ~ rotated -90 0 run function big_kitchen:warp
execute as @a[tag=pr.target] at @s if block ~ ~-2 ~ minecraft:birch_wood run tag @s add big_kitchen.warp2

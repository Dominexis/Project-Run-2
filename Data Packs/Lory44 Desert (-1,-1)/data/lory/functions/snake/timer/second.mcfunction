...
execute unless score $snake_timer.tmp lory.var matches 1.. at @e[type=minecraft:marker,distance=0..32,tag=lory.snake_part,tag=pr.target] run particle minecraft:flame ~ ~ ~ 0.0001 0.0001 0.0001 0 1 normal @a
...
execute unless score $snake_timer.tmp lory.var matches 1.. as @e[type=minecraft:marker,distance=..32,tag=lory.snake_part,tag=pr.target] at @s run function lory:snake/set

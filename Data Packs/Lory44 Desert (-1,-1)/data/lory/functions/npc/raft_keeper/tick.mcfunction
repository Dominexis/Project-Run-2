...
execute as @a[distance=..6,tag=!lory.mission_raft,tag=pr.target,scores={lory.npc.raft_keeper.cooldown=1..}] run function lory:npc/raft_keeper/dialogue/mission

scoreboard players set #input pr.value 31
function pr:generic/rng/predicate

execute as @a[distance=..6,tag=lory.mission_raft,tag=pr.target,scores={lory.npc.raft_keeper.cooldown=..0}] run function lory:npc/raft_keeper/dialogue/random
...
scoreboard players set @a[distance=6..,tag=pr.target,tag=lory.mission_raft] lory.npc.raft_keeper.cooldown 21

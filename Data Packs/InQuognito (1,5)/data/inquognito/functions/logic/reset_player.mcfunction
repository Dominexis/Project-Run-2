advancement revoke @s through inquognito:groups/all

scoreboard players reset @s inquognito.id

scoreboard players reset @s inquognito.jump
scoreboard players reset @s inquognito.use

scoreboard players reset @s inquognito.energy
scoreboard players reset @s inquognito.health
scoreboard players reset @s inquognito.suit
scoreboard players reset @s inquognito.upgrades

scoreboard players reset @s inquognito.dialogue
scoreboard players reset @s inquognito.dialogue.tick
scoreboard players reset @s inquognito.boss.health
scoreboard players reset @s inquognito.boss.heartbeat

scoreboard players reset @s inquognito.clearance
scoreboard players reset @s inquognito.clearance.1.codes
scoreboard players reset @s inquognito.clearance.bounce_pad_room
scoreboard players reset @s inquognito.clearance.energy
scoreboard players reset @s inquognito.clearance.frostbite
scoreboard players reset @s inquognito.clearance.office
scoreboard players reset @s inquognito.clearance.office.1
scoreboard players reset @s inquognito.clearance.office.2
scoreboard players reset @s inquognito.clearance.office.3
scoreboard players reset @s inquognito.clearance.sector_4

scoreboard players reset @s inquognito.transmission
scoreboard players reset @s inquognito.transmission.current
scoreboard players reset @s inquognito.transmission.tick

scoreboard players reset @s inquognito.doubleJump.cooldown

scoreboard players reset @s inquognito.gun.augments
scoreboard players reset @s inquognito.gun.cooldown
scoreboard players reset @s inquognito.gun.damage
scoreboard players reset @s inquognito.gun.fireRate

scoreboard players reset @s inquognito.jetpack.cooldown
scoreboard players reset @s inquognito.jetpack.duration
scoreboard players reset @s inquognito.jetpack.fuel

tag @s remove inquognito.id_set

tag @s remove inquognito.doubleJumpUsed

tag @s remove inquognito.gun
tag @s remove inquognito.gun.frostbite
tag @s remove inquognito.gun.nova
tag @s remove inquognito.gun.trinity

tag @s remove inquognito.boss.start
tag @s remove inquognito.boss.darkness
tag @s remove inquognito.boss.darkness_particles

tag @s remove inquognito.win

tag @s remove inquognito.code.1
tag @s remove inquognito.code.2
tag @s remove inquognito.code.3

tag @s remove inquognito.clearance.sewer
tag @s remove inquognito.clearance.nova

tag @s remove inquognito.target.bounce_pad_room.1
tag @s remove inquognito.target.bounce_pad_room.2
tag @s remove inquognito.target.bounce_pad_room.3
tag @s remove inquognito.target.bounce_pad_room.4

tag @s remove inquognito.target.energy

tag @s remove inquognito.target.frostbite.1
tag @s remove inquognito.target.frostbite.2
tag @s remove inquognito.target.frostbite.3

tag @s remove inquognito.target.office
tag @s remove inquognito.target.office.1
tag @s remove inquognito.target.office.2
tag @s remove inquognito.target.office.3

clear @s

...
#RNG HAPPEN
execute as @a[tag=pr.target,tag=!JP_RNG] if block ~ ~ ~ polished_blackstone_pressure_plate run loot spawn ~-31 ~-60 ~34 loot jpmagic:jump_rng
execute positioned ~-31 ~-60 ~34 as @e[type=item,tag=pr.target,distance=..2] as @a[tag=pr.target,tag=!JP_RNG] if block ~ ~ ~ polished_blackstone_pressure_plate run scoreboard players add @s jpmagic.tmp 1
kill @e[type=item,tag=pr.target]
execute as @a[tag=pr.target,tag=!JP_RNG] if block ~ ~ ~ polished_blackstone_pressure_plate run function jpmagic:jprng
execute as @a[tag=pr.target] if block ~ ~ ~ polished_blackstone_pressure_plate run tag @s add JP_RNG
execute as @a[tag=pr.target] unless block ~ ~ ~ polished_blackstone_pressure_plate run tag @s remove JP_RNG


#BOSS FIGHT
#Item give
execute at @e[tag=pr.target,tag=jp_key_back] as @a[tag=pr.target,distance=..5] run item replace entity @s hotbar.0 with golden_sword{AttributeModifiers:[{AttributeName:"generic.attack_speed",Amount:10,Slot:mainhand,Name:"generic.attack_speed",UUID:[I;-123515,30979,13382,-61958]}],Unbreakable:1,display:{Name:'[{"text":"Mid","italic":false,"color":"#fe5a1c"},{"text":"as\'","color":"#fe7c22"},{"text":" ","color":"#fe7c22"},{"text":"Exc","color":"#ff9e28"},{"text":"ali","color":"#ffbf2e"},{"text":"bur","color":"#ffe134"}]'},Enchantments:[{id:sharpness,lvl:10}],HideFlags:1} 1
execute at @e[tag=pr.target,tag=jp_key_back] as @a[tag=pr.target,distance=..5] run item replace entity @s hotbar.2 with acacia_boat{display:{Name:'[{"text":"Magma Key","italic":false,"color":"light_purple"}]',Lore:['[{"text":"Some things are better left","italic":true,"color":"gray"}]','[{"text":"unknown...","italic":true,"color":"gray"},{"text":"","italic":false,"color":"dark_purple"}]','[{"text":"","italic":false,"color":"dark_purple"}]','[{"text":"MYTHIC","italic":false,"color":"light_purple","bold":true}]']}} 1
...
#Key Detection
execute at @e[tag=pr.target,tag=jp_key_back] run tp @a[tag=pr.target,distance=..5,predicate=jpmagic:holding_tripwire_hook] ~ ~ ~-2
execute at @e[tag=pr.target,tag=jp_key_back] as @a[tag=pr.target,distance=..5,predicate=jpmagic:holding_tripwire_hook] run function jpmagic:lavadoor
...
execute positioned ~-6 ~70 ~-2 as @a[tag=pr.target,distance=..2] at @s if entity @s if block ~ ~-1 ~ minecraft:slime_block run effect give @s minecraft:levitation 1 127 true
execute positioned ~-6 ~70 ~-2 as @a[tag=pr.target,distance=..2] at @s run clear @s
execute positioned ~-6 ~70 ~-2 as @a[tag=pr.target,distance=..4] at @s run function pr:player/finish
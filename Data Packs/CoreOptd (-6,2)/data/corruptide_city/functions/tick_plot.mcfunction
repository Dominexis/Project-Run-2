...
execute as @a[tag=coretidecity_player_tag,tag=pr.target] at @s if block ~ ~-1 ~ lightning_rod run kill @s
execute as @a[tag=coretidecity_player_tag,tag=pr.target] at @s if block ~ ~-1 ~ end_rod run kill @s
execute as @a[tag=coretidecity_player_tag,tag=pr.target] at @s if block ~ ~-0.5 ~ iron_bars run kill @s
#ELEVATOR COMMANDS
execute as @e[tag=corecity.elevator_start,type=armor_stand] at @s run tp @a[tag=coretidecity_player_tag,tag=pr.target,distance=..2] ~-2 ~-27.5 ~-1 -90 0
execute as @e[tag=corecity.elevator_end,type=armor_stand] at @s run tp @a[tag=coretidecity_player_tag,tag=pr.target,distance=..2] ~ ~41.5 ~ -90 0
#ENDING COMMANDS
execute positioned ~-39 ~84 ~0 as @a[distance=..2,sort=nearest,limit=1] if block ~ ~ ~ minecraft:mangrove_button[powered=true] run playsound minecraft:entity.generic.explode master @s ~ ~ ~ 1 0.1
execute positioned ~-39 ~84 ~0 as @a[distance=..2,sort=nearest,limit=1] if block ~ ~ ~ minecraft:mangrove_button[powered=true] run playsound minecraft:block.beacon.power_select master @s ~ ~ ~ 1 2
execute positioned ~-39 ~84 ~0 as @a[distance=..2,sort=nearest,limit=1] if block ~ ~ ~ minecraft:mangrove_button[powered=true] run function pr:player/finish
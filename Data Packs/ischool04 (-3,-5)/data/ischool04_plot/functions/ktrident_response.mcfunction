...
kill @s
execute as @a[tag=pr.target,nbt={Inventory:[{id:"minecraft:trident"}]}] unless entity @s run give @s trident{display:{Name:'[{"text":"Infernal Destroyer","italic":false,"bold":true,"color":"dark_red"}]',Lore:['[{"text":"[","italic":false,"color":"dark_blue"},{"text":"Distance Weapon","color":"blue"},{"text":"]"}]']},Enchantments:[{id:"loyalty",lvl:3}],HideFlags:131} 1
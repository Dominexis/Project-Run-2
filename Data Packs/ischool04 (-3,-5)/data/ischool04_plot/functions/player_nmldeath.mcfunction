execute at @s at @e[tag=Rzg,limit=1,sort=nearest,tag=pr.target] run tp @e[tag=Rzg,limit=1,sort=nearest,tag=pr.target] ~ ~-100 ~
...
execute at @s run summon zoglin ~ ~ ~ {HasVisualFire:0b,Silent:0b,Invulnerable:1b,CustomNameVisible:1b,DeathLootTable:"air",NoAI:1b,Tags:["Rzg"],CustomName:'{"text":"Bloody Beast","color":"white","bold":true}',ActiveEffects:[{Id:5,Amplifier:1b,Duration:999999,ShowParticles:1b}],Attributes:[{Name:generic.knockback_resistance,Base:1}]}
...
execute at @s run ride @s mount @e[tag=Czg,limit=1,sort=nearest,tag=pr.target]
